...
import { 
  ArrowLeft, FileText, CheckCircle, AlertTriangle, Play, Sparkles, RefreshCw, Check 
} from 'lucide-react';
...
export default function PdfExtractWorkspace({ onBack }) {
  const [selectedFile, setSelectedFile] = useState(null);
  const [pagesList, setPagesList] = useState([]); // [{ index, thumbUrl }]
  const [selectedPages, setSelectedPages] = useState([]); // Array of 0-indexed indices selected for extraction
    ...
    const fetchLimits = async () => {
      try {
        const response = await api.get('/tools/limits', {
          params: { toolSlug: 'pdf-extract-pages' }
        });
  ...
  const handleExtractPages = async () => {
    ...
    try {
      ...
      // Sort indices ascending to extract pages in natural sequence order
      const sortedIndices = [...selectedPages].sort((a, b) => a - b);

      const copiedPages = await extractedDoc.copyPages(pdfDoc, sortedIndices);
      copiedPages.forEach((page) => extractedDoc.addPage(page));

      const finalBytes = await extractedDoc.save();
      ...
      setSuccessMessage(`Successfully extracted ${selectedPages.length} pages and generated new PDF.`);
      setSelectedPages([]);

      // Log usage metrics
      try {
        await api.post('/tools/log', { toolSlug: 'pdf-extract-pages' });
      ...
      setErrorMessage(err.message || 'Error occurred while extracting PDF pages.');
    } finally {
        ...
        <div className="p-5 rounded-3xl bg-amber-500/10 border border-amber-500/20 text-amber-500 flex items-start gap-4 text-sm leading-relaxed mb-6">
          ...
          <div>
            ...
            <p className="mt-1 text-slate-400">
              Please sign in or upgrade to premium to extract PDF pages.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl flex flex-col md:flex-row gap-6 items-center">
            <div className="text-center md:text-left min-w-0 w-full">
              ...
              <span className="block text-xs font-semibold text-slate-400 mt-1">
                Pages: {pagesList.length} • Selected for extraction: <span className="text-violet-500 font-bold">{selectedPages.length}</span>
              </span>
            </div>
              ...
              return (
                <div
                  ...
                  className={`group relative bg-white dark:bg-slate-900 border rounded-2xl p-4 text-center cursor-pointer transition shadow-sm ${
                    isSelected ? 'border-violet-500/60 bg-violet-500/5' : 'border-slate-200 dark:border-slate-800 hover:border-violet-500/40'
                  }`}
                >
                  <div className="relative aspect-[3/4] rounded-lg overflow-hidden border border-slate-100 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-950 flex items-center justify-center">
                    <img
                      ...
                      className={`max-h-full max-w-full object-contain transition-opacity ${isSelected ? 'opacity-60' : ''}`}
                    />
                    
                    {/* Selected badge overlay */}
                    {isSelected && (
                      <div className="absolute inset-0 bg-violet-600/10 border border-violet-500/25 flex items-center justify-center">
                        <div className="p-2.5 rounded-full bg-violet-600 text-white shadow-lg flex items-center justify-center">
          ...
          <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-6">
            ...
            >
              {isProcessing ? (
                ...
                <>
                  <FileText className="h-4 w-4" /> Extract Selected Pages
                </>
              )}
            </button>
          </div>
